--ALTER VIEW evolve.TIL_OOC_REVIEW AS
WITH 
wellbore AS (
SELECT Well_UID
      ,GIS_Link AS Parcel
...
SELECT lui1.Well_UID AS Well_ID
		...
		,SS_StartFirstSales AS TIL_Date
		,Unit_UID AS Unit_ID
...
WHERE lui1.Intersect_Length > 25 AND wl.Status NOT In ('9_Abandoned','8_Temporary Abandoned','7_Producing','6_Frac Completed','10_Long Term Shut-In','0_Tentative') AND WellStage = 'Lateral'
),
...
GROUP BY WellName, Well_ID, TIL_Date, mors.GIS_LINK, wellbore.Wellbore_Status, Intersect_Acreage, Percent_In_Unit
...
LEFT JOIN (SELECT * from Wells
           LEFT JOIN (SELECT GIS_Link
                      ...
                      ) AS mors ON mors.GIS_UID = wells.Unit_ID
...
SELECT WRPV.*
    , Milestones.Start
    , dm.Dev_Run_Name AS Devrun
    , dm.Start AS Devrun_Start
    , ROW_Number() OVER(Partition by Adjusted_Parcel_Number ORDER By Milestones.Start) AS row
FROM Well_Related_Parcel_view WRPV
RIGHT JOIN heart.r_schedule_milestones AS Milestones ON W_UID = Well_ID AND Milestone = 'First_Sales' AND Start IS NOT Null
LEFT JOIN  heart.r_devrun_milestones AS dm ON dm.Dev_Run_UID = Milestones.Dev_Run_UID
...
devrun_dates AS (
SELECT Adjusted_Parcel_Number AS Parcel_Number
      ,Devrun_Start AS Earliest_Devrun_Date
      ,Devrun AS Earliest_Devrun
      ,WellName AS Earliest_Well
FROM (SELECT DISTINCT Adjusted_Parcel_Number, WellName, Devrun, Devrun_Start from WRPV_FINAL where row = 1) T
)
,
date_info AS (
SELECT Adjusted_Parcel_Number AS Parcel_Number
      ,MIN(Start) AS Earliest_Date
      ,String_AGG(cast(Start AS varchar(max)), ',') AS TIL_Dates
	  ,CONVERT(date, DATEADD(yy, -3, MIN(Start)), 103) AS OOC_Evaluation_Date
      ,CONVERT(date, DATEADD(dd, -75, MIN(Start)), 103) AS OOC_Review_Date
      ,CONVERT(date, DATEADD(dd, -60, MIN(Start)), 103) AS Short_Term_Target_Review_Date
...
WHERE ors.RecordTypeId NOT In ('0125000000022zNAAQ','0125000000022zSAAQ')
)
,ORPP
AS
(
...
AS
(
SELECT CAST(AVG(Avg_ORs_Per_Active_Day) AS DECIMAL(6,2)) AS Average_ORs_Per_Person_Per_Day
FROM
(
...
WHERE ors.CreatedDate > '1/1/2021' AND accts.Title LIKE ('%Title%') AND accts.Name NOT IN ('Danielle St Onge','Amanda Godwin (Inactive)','Bethany Parker (Inactive)','Ryan Wagley') AND NOT (ors.RecordTypeId = '0125000000022zNAAQ' OR ors.RecordTypeId = '0125000000022zSAAQ') 
...
SELECT
    ...
    ,TB.OOC_Review_Date
    ,YEAR(TB.Short_Term_Target_Review_Date) AS ST_Target_Year
