...
SELECT CASE WHEN mors.GIS_LINK LIKE ('%[_]%') THEN SUBSTRING(mors.GIS_LINK, 1, (LEN(mors.GIS_LINK)-2))
      ...
      ,CASE WHEN Wellbore_status = 'Swing' THEN 'Air Curve'
	   ...
	   END AS Wellbore_Status
FROM wellbore wb
LEFT JOIN mors ON mors.GIS_LINK = wb.Parcel AND mors.Well_ID = wb.Well_UID
WHERE WellName IS NULL
),
wbco AS
        (SELECT 
            wrp.GIS_EX_ID__c AS Parcel_Number
            ,   wrp.Title_Workflow_Status__c
            ,   WBCOMacroStatus__c
            ,   wbco.Related_Well_GIS_UID__c 
        FROM heart.sf_wellrelatedparcel__c wrp
        LEFT join heart.sf_well_bore_dev_run_clearance__c wbco ON wrp.RelatedWellBoreDevRunClearance__c = wbco.id)
...
SELECT 
    ...
    , ROW_Number() OVER(Partition by Adjusted_Parcel_Number ORDER By well_sched.Start) AS row
FROM Well_Related_Parcel_view WRPV
RIGHT JOIN heart.r_schedule_milestones AS well_sched ON W_UID = Well_ID AND Milestone = 'Horizontal_Drilling' AND well_sched.Start IS NOT Null
LEFT JOIN  heart.r_devrun_milestones AS dm ON dm.Dev_Run_UID = well_sched.Dev_Run_UID AND dm.Milestone = 'Horizontal_Drilling'
RIGHT JOIN wbco 
    ON (Well_ID = wbco.Related_Well_GIS_UID__c AND WBCOMacroStatus__c NOT LIKE ('3%') AND WBCOMacroStatus__c NOT LIKE ('4%')) 
    OR (wbco.Parcel_Number = wrpv.Adjusted_Parcel_Number AND NOT (Title_Workflow_Status__c IN ('Title Complete','Not Reviewed')))
WHERE wrpv.PARCEL_NUMBER IS NOT NULL
),
split_parcels AS (
SELECT GIS_EX_ID__c,
